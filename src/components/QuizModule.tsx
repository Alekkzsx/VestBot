...
const QuizModule: React.FC = () => {
  ...
  const startQuiz = async () => {
    setStatus('LOADING');
    try {
      const qs = await generateQuiz(selectedSubject, 5);
      setQuestions(qs);
      ...
      showToast('error', 'Erro ao gerar simulado. Verifique sua conexão e tente novamente.');
      setStatus('SETUP');
    }
  };
  ...
  if (status === 'LOADING') {
    return (
      <div className="flex flex-col items-center justify-center h-96">
        ...
        <p className="text-gray-600 animate-pulse">A IA está elaborando suas questões estilo ETEC...</p>
      </div>
    );
  }
          ...
          <div className="space-y-3">
            {q.options.map((opt, idx) => (
              <button
                ...
                className={`w-full text-left p-4 rounded-xl border-2 transition-all ${userAnswers[currentQIndex] === idx
            ...
            <div key={q.id} className={`p-6 rounded-xl border ${isCorrect ? 'border-green-200 bg-green-50/50' : 'border-red-200 bg-red-50/50'}`}>
              ...
              <div className="space-y-2 ml-9 mb-4 text-sm">
                {q.options.map((opt, optIdx) => {
                  ...
                  if (optIdx === q.correctIndex) style = "bg-green-100 text-green-800 font-medium border border-green-200 p-2 rounded";
                  else if (optIdx === userAnswerIdx && !isCorrect) style = "bg-red-100 text-red-800 line-through opacity-75 p-2 rounded";

...
import { StudyPlan, ETECCourse, Subject } from '../types';
...
const StudyPlanModule: React.FC = () => {
  ...
  // Form State
  const [course, setCourse] = useState<ETECCourse>(ETECCourse.INFORMATICS);
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const result = await generateStudyPlan(course, hours, selectedWeaknesses);
      setPlan(result);
      showToast('success', 'Plano de estudos gerado com sucesso!');
    } catch (error) {
      console.error('Error generating plan:', error);
      showToast('error', 'Erro ao gerar plano. Verifique sua conexão e tente novamente.');
    } finally {
      setLoading(false);
    }
  };
      ...
      {!plan ? (
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Qual curso você quer?</label>
              <select
                value={course}
                onChange={(e) => setCourse(e.target.value as ETECCourse)}
                className="w-full p-3 border rounded-lg bg-gray-50 focus:ring-2 focus:ring-indigo-500 outline-none"
              >
                {Object.values(ETECCourse).map((t) => (
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.values(Subject).map((sub) => (
                  <button
                    ...
                    className={`text-xs px-3 py-2 rounded-lg border transition-all ${selectedWeaknesses.includes(sub)
        ...
        <div className="animate-fade-in">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-semibold text-gray-700">Plano Semanal: {plan.targetCourse}</h3>
